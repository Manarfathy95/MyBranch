        ...
        if(likes.is(':visible'))
            {
                ...
                this.get_class_educator_id().then(() => {
                    cy.task('NagwaClassesDB', `SELECT
            e.educator_id,
            SUM(sf.session_feedback) as session_feedback,
            CAST(CAST(SUM(sf.session_feedback) AS DECIMAL)/CAST(COUNT(sf.session_feedback) AS DECIMAL) AS DECIMAL(18,2))*100 as session_feedback_percentage
        FROM
            educators e
        JOIN sessions s ON
            e.educator_id = s.educator_id
            AND s.is_test_session =false
        JOIN sessions_feedback sf ON
            s.session_id = sf.session_id
        JOIN students st on
            st.student_id =sf.student_id
            AND st.is_test_student =FALSE
        WHERE e.educator_id=${this.educator_ID}
        GROUP BY e.educator_id;`).then((res) => {
        
                    
                   
                   cy.verfiyTheDisplayedText(this.elements.educator_percentage(), Number(res[0].session_feedback_percentage)+ '%')
                   //Number() >> this is to convert from decimal to number 
                   //% >> to add to the string  
        
                    cy.verfiyTheDisplayedText(this.elements.educator_likes(), res[0].session_feedback)
